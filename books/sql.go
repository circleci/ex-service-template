...
func queryGetAllBooks(ctx context.Context, q db.Querier) (books []Book, err error) {
	ctx, span := db.Span(ctx, "service", "query_get_all_books")
	defer o11y.End(span, &err)

	err = q.SelectContext(ctx, &books, getAllBooksSQL)
	if err != nil {
		return nil, err
	}
