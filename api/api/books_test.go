...
func TestAPI_getAllBooks(t *testing.T) {
	...
	t.Run("Success", func(t *testing.T) {
		var ids []uuid.UUID
		assert.Assert(t, t.Run("Add books to store", func(t *testing.T) {
			var err error
			ids, err = fix.Store.AddMany(ctx, []books.ToAdd{
				{
					Name:  "Wizard of Oz",
					Price: "$6.99",
				},
		...
		t.Run("Check books can be found", func(t *testing.T) {
			var m []map[string]interface{}
			status := fix.Get(t, "/api/books", &m)
			assert.Check(t, cmp.Equal(status, http.StatusOK))
			assert.Check(t, cmp.DeepEqual([]map[string]interface{}{
				{
					"id":    ids[0].String(),
