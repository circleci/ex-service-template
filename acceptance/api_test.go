...
import (
	...
	"github.com/circleci/ex-service-template/migrations"
)
...
func TestE2E(t *testing.T) {
	...
	t.Run("Test ping pong", func(t *testing.T) {
		...
		assert.Assert(t, t.Run("Ping to give Pong back", func(t *testing.T) {
			var res response
			status := fix.Get(t, "/api/ping", &res)
			assert.Check(t, cmp.Equal(status, http.StatusOK))
			assert.Check(t, cmp.Equal(res.Message, "pong"))
		}))
	})

	t.Run("Test books endpoint", func(t *testing.T) {
		type response struct {
